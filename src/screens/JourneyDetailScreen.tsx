  ...
  if (loading || !journey) {
      return (
          <View style={styles.loadingContainer}>
              ...
              <Text style={{marginTop: 16, color: '#718096'}}>Recalling memory...</Text>
          </View>
      );
  }
    ...
    <ScrollView style={styles.container}>
      <View style={styles.mapContainer}>
        <MapView
            ...
            initialRegion={journey.coordinates && journey.coordinates.length > 0 ? {
      ...
      <View style={styles.content}>
        <Text style={styles.date}>{format(new Date(journey.start_time), 'MMMM do, yyyy')}</Text>
        <Text style={styles.title}>{journey.title || 'Untitled Journey'}</Text>

        <View style={styles.statsRow}>
            <View style={styles.statItem}>
                <MapPin size={20} color="#718096" />
                <Text style={styles.statText}>{(journey.distance_meters / 1000).toFixed(2)} km</Text>
            </View>
            <View style={styles.statItem}>
                <Clock size={20} color="#718096" />
                <Text style={styles.statText}>{formatDuration(journey.duration_seconds)}</Text>
            </View>
        </View>
        ...
        <Text style={styles.memoryText}>"{journey.memory_text || 'No words written.'}"</Text>

        <View style={styles.reflectionPrompt}>
            <Text style={styles.promptText}>You walked this path {format(new Date(journey.start_time), 'PP')}.</Text>
        </View>
