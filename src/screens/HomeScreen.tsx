...
export default function HomeScreen({ navigation }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (!error && journeys) {
        const totalDist = journeys.reduce((acc, curr) => acc + (curr.distance_meters || 0), 0);
        setStats({
    ...
    const { data: recent, error: recentError } = await supabase
        .from('journeys')
        .select('id, title, distance_meters, duration_seconds, created_at, start_time')
        .eq('user_id', user.id)
          ...
          recentJourneys.map((journey) => (
            ...
            >
                <View style={styles.journeyHeader}>
                    ...
                    <Text style={styles.journeyDate}>{formatDistanceToNow(new Date(journey.start_time), { addSuffix: true })}</Text>
                </View>
                <View style={styles.journeyFooter}>
                    <View style={styles.journeyStat}>
                        <MapPin size={14} color="#718096" />
                        <Text style={styles.journeyStatText}>{(journey.distance_meters / 1000).toFixed(2)} km</Text>
                    </View>
                    <View style={styles.journeyStat}>
                        <Clock size={14} color="#718096" />
                        <Text style={styles.journeyStatText}>{Math.floor(journey.duration_seconds / 60)} mins</Text>
                    </View>
                </View>
            </TouchableOpacity>
          ))
